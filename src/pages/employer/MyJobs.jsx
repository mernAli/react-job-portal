  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${theme.cardBg} p-6 rounded-xl ${theme.border} border`}>
        <h1 className={`text-2xl font-bold ${theme.textPrimary}`}>My Job Postings</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
        <div className={`${theme.cardBg} p-6 rounded-xl ${theme.border} border`}>
          ...
          <h3 className={`text-3xl font-bold ${theme.textPrimary} mt-2`}>{jobs.length}</h3>
        </div>
        <div className={`${theme.cardBg} p-6 rounded-xl ${theme.border} border`}>
          <p className={`text-sm ${theme.textMuted} font-medium`}>Active Jobs</p>
          <h3 className={`text-3xl font-bold ${theme.successText} mt-2`}>
            {jobs.filter((j) => j.status === "Active").length}
          </h3>
        </div>
        <div className={`${theme.cardBg} p-6 rounded-xl ${theme.border} border`}>
          <p className={`text-sm ${theme.textMuted} font-medium`}>Total Applicants</p>
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
              <div className="flex-1">
                ...
                <div className={`flex flex-wrap items-center gap-3 lg:gap-4 text-sm ${theme.textSecondary}`}>
                ...
                <div className={`flex flex-wrap items-center gap-3 lg:gap-4 mt-3 text-sm`}>
              ...
              <div className="flex items-center gap-3 lg:flex-col lg:items-end">
                ...
                <div className="flex gap-2 flex-1 lg:flex-none">
                  <button className={`flex-1 lg:flex-none px-4 py-2 text-sm ${theme.primaryText} ${theme.border} border rounded-lg ${theme.hover} font-medium whitespace-nowrap`}>
                    Edit
                  </button>
                  <button className={`flex-1 lg:flex-none px-4 py-2 text-sm ${theme.dangerText} ${theme.border} border rounded-lg ${theme.hover} font-medium whitespace-nowrap`}>
                    Close
                  </button>
                </div>
              </div>
